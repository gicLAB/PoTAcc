  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        // save the input
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_input.csv",
        //     gemm_input_data, gemm_input_cols, gemm_input_rows);

        // save the filter
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_filter.csv",
        //     filter_data, filter_rows, filter_cols);

        // save wb[i]
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_wb0.csv",
        //     wb_0, (filter_rows / 4), filter_cols);
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_wb1.csv",
        //     wb_1, (filter_rows / 4), filter_cols);
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_wb2.csv",
        //     wb_2, (filter_rows / 4), filter_cols);
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_wb3.csv",
        //     wb_3, (filter_rows / 4), filter_cols);
        // save the output
        // saveMatrixCSV(
        //     "/home/ubuntu/Workspace/secda_apps_evaluation_suite/tmp/" +
        //         std::to_string(associated_nodes[i]) + "_exact_out_acc.csv",
        //     output_data, gemm_input_cols, filter_rows);
...
public:
  ...
  bool IsNodeSupportedByDelegate(const TfLiteRegistration *registration,
    ...
    // specific to hedl delegate
    // first layer (cov2d for cnns) is not delegated
    static int first_layer_no = 0;
    if (isCONV2D && first_layer_no == 0) {
      first_layer_no++;
      if (first_layer_no == 1) {
        return false;
      }
    }
    // end specific to hedl delegate
...
void TfLiteVMDelegateDelete(TfLiteDelegate *delegate) {
  ...
  // SYSC_ON(profile.saveCSVRecords("adata/vm_profs/vm_" + date));
