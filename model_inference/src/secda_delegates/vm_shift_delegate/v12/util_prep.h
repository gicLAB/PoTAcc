...
int8_t roundToNearestPowerOf2(int8_t x) {
  ...
  if (sign == 1 && x != 0) return (int8_t)result;
...
// expected APoT weights are
// int8_t apot_levels_opt[15] =
...
// next 2 bits
// 00 --> 0
// 01 --> eta (zero multiplier)
// 10 --> 2
// 11 --> 3
// next 1 bit
// 0 --> eta (zero multiplier)
// 1 --> 1

int8_t apot_weight_encoding(int8_t x) {
  int sign = (x > 0) ? 1 : -1;
  int abs_x = abs(x);
  int result = 0;

  if (abs_x == 0) result = 2;       // 01 0
  else if (abs_x == 1) result = 0;  // 00 0
  else if (abs_x == 2) result = 3;  // 01 1
  else if (abs_x == 3) result = 1;  // 00 1
  else if (abs_x == 4) result = 4;  // 10 0
  else if (abs_x == 6) result = 5;  // 10 1
  else if (abs_x == 8) result = 6;  // 11 0
  else if (abs_x == 10) result = 7; // 11 1
  else
    // assert error
    assert("Invalid APoT weight found");

  if (sign == -1) result = result | 0x8; // set sign bit
  return (int8_t)result;
}

// TODO: this is just a copy of apot
// needs to be updated
int8_t msq_weight_encoding(int8_t x) {
  int sign = (x > 0) ? 1 : -1;
  int abs_x = abs(x);
  int result = 0;
...
// !!!! Data requantization
// will return the scale of the requantize_Data
// max_Data_level: 127
// max_rqData_level: 10 for 4-bit APoT weights and 8 for MSQ weights
double requantize_int8(const int8_t *data = nullptr, const double DScale = 1.0,
                       int8_t *rqData = nullptr, int *dims = nullptr,
                       const int8_t max_Data_level = 127,
                       const int8_t max_rqData_level = 10,
                       const int8_t *pot_levels = nullptr,
                       int pot_levels_size = 15) {
  // implement the data requantization logic here

  int dWidth = dims[0];
  int dDepth = dims[1] * dims[2] * dims[3];
  double rqData_scale = DScale * (static_cast<double>(max_Data_level) /
                                  static_cast<double>(max_rqData_level));
  for (int i = 0; i < (dWidth * dDepth); i++) {
    // dequantize the data
    double dequantized_value = static_cast<double>(data[i]) * DScale;
    // quantize the data using rqData_scale
    double quantized_value = dequantized_value / rqData_scale;
    // round to nearest pot_levels
    rqData[i] =
        roundToNearestPOTLevels(quantized_value, pot_levels, pot_levels_size);
  }
  // cout << "Div Factor: " << rqData_scale / DScale << endl;
  return rqData_scale;
}

// Index based requantization
double requantize_potInt(const int8_t *data = nullptr,
                         const double DScale = 1.0, int8_t *rqData = nullptr,
                         int *dims = nullptr, const int8_t max_Data_level = 127,
                         const int8_t max_rqData_level = 10,
                         const int8_t *pot_levels = nullptr,
                         int pot_levels_size = 15,
                         const int8_t *pot_opt_levels = nullptr,
                         int pot_opt_levels_size = 15) {

  // check pot_levels_size and pot_opt_levels_size
  assert(pot_levels_size == pot_opt_levels_size &&
         "Error: requantize_potInt(): pot_levels_size and pot_opt_levels_size "
         "must be equal");
  // implement the data requantization logic here
  int dWidth = dims[0];
  int dDepth = dims[1] * dims[2] * dims[3];
  ...
  double rqData_scale = DScale * (static_cast<double>(max_Data_level) /
                                  static_cast<double>(max_rqData_level));
  for (int i = 0; i < (dWidth * dDepth); i++) {
    // find the index of the data in the pot_levels list
    int pot_level_idx = -1;
    for (int idx = 0; idx < pot_levels_size; ++idx) {
      if (data[i] == pot_levels[idx]) {
        pot_level_idx = idx;
        break;
      }
    }

    // find the value in pot_opt_levels according to the index
    if (pot_level_idx != -1) {
      rqData[i] = pot_opt_levels[pot_level_idx];
    } else {
      assert(false &&
             "Error: requantize_potInt(): pot_level_idx not found for data[i]");
    }
  }
  // debug info
  // cout << "original scale: " << DScale << " Requantized scale: " <<
  // rqData_scale
  //      << endl;
  return rqData_scale;
}

int find_pot_level_index(const int8_t *wgt_arr, int *dims,
                         const double filter_scale, const int8_t *pot_levels,
                         int pot_levels_size) {
  int pot_level_index = -1;
  std::vector<int8_t> max_diffs_vec;
  // clear the vecs
  max_diffs_vec.clear();

  // filter dims
  int filter_w = dims[0];
  int filter_d = dims[1] * dims[2] * dims[3];

  // we assume pot_levels are symmetric around 0
  int pot_levels_length = pot_levels_size / 2;

  // create a vector to store the requantized int8_t filter with the
  // temp_pot_scale
  vector<int8_t> requantize_int8_filter(filter_w * filter_d);

  // loop through pot_levels
  // creating scale with i == 0 pot_levels[0], is same as filter_scale from
  // tflite
  for (int i = 1; i < pot_levels_length; ++i) {
    double temp_pot_scale =
        (filter_scale * 127.0) / (static_cast<double>(pot_levels[i]) * (-1.0));

    // dequantize and requantize
    for (int j = 0; j < filter_w; ++j) {
      for (int k = 0; k < filter_d; ++k) {
        int index = j * filter_d + k;
        // dequantize the data
        ...
        int quantized =
            static_cast<int>(round(dequantized_value / temp_pot_scale));

        // clamp to pot_levels, pot_levels[i] is a negative value
        if (quantized > pot_levels[i] * (-1)) {
          quantized = pot_levels[i] * (-1);
        }
        if (quantized < pot_levels[i]) {
          quantized = pot_levels[i];
        }
        int8_t quantized_value = static_cast<int8_t>(quantized);
        requantize_int8_filter[index] = quantized_value;
      }
    }
    // find the unique values from the requantized filter
    std::set<int8_t> unique_requantized_values(requantize_int8_filter.begin(),
                                               requantize_int8_filter.end());
    // create a vector from the set
    std::vector<int8_t> unique_requantized_values_vec(
        unique_requantized_values.begin(), unique_requantized_values.end());
    // create 4d dim for unique_requantized_values_vec
    int *unique_requantized_dims = new int[4];
    unique_requantized_dims[0] = 1; // batch size
    unique_requantized_dims[1] = 1; // height
    unique_requantized_dims[2] = 1; // width
    unique_requantized_dims[3] =
        unique_requantized_values_vec.size(); // channel
    // check if the unique_requantized_values_vec is within the pot_levels
    bool is_this_potFilter =
        check_pot_weights(unique_requantized_values_vec.data(),
                          unique_requantized_dims, pot_levels, pot_levels_size);
    if (is_this_potFilter) {
      pot_level_index = i;
      return pot_level_index; // return the pot_level_index
    } else {
      // check : unique_requantized_values_vec
      // mismatches: values not in pot_levels
      // closest_pot_levels: closest values in pot_levels of mismatches
      // diffs: differences between mismatches and closest_pot_levels
      // min_diffs: min differences between mismatches and closest_pot_levels
      // max_diffs: max differences between mismatches and closest_pot_levels
      std::vector<int8_t> mismatches;
      std::vector<int8_t> closest_pot_levels;
      std::vector<int8_t> diffs;
      int8_t min_diff = 0;
      int8_t max_diff = 0;
      for (int8_t val : unique_requantized_values_vec) {
        if (std::find(pot_levels, pot_levels + pot_levels_size, val) ==
            pot_levels + pot_levels_size) {
          mismatches.push_back(val);
          // find the closest pot_levels
          int8_t closest = 0;
          for (int i = 0; i < pot_levels_size; ++i) {
            if (std::abs(pot_levels[i] - val) < std::abs(closest - val)) {
              closest = pot_levels[i];
            }
          }
          closest_pot_levels.push_back(closest);
          diffs.push_back(val - closest);
        }
      }
      if (!diffs.empty()) {
        min_diff = *std::min_element(diffs.begin(), diffs.end());
        max_diff = *std::max_element(diffs.begin(), diffs.end());
      }
      // store the mismatches_no, min_diff and max_diff in the vecs
      max_diffs_vec.push_back(max_diff);
    }
  }

  // we are depending on the max_diff to find the pot_level_index
  int idx_maxDiff =
      std::min_element(max_diffs_vec.begin(), max_diffs_vec.end()) -
      max_diffs_vec.begin();
  pot_level_index = idx_maxDiff + 1; // +1 because we started from 1
  // debug info
  // cout << "For PoT levels: [" << (int)pot_levels[idx_maxDiff + 1]
  //      << "] with small maxDiff: " << (int)max_diffs_vec[idx_maxDiff]
  //      << " idx: " << idx_maxDiff + 1 << endl;
  return pot_level_index;
}

void prepare_pot_weights(TfLiteTensor *wgt, int8_t *wgt_pot = nullptr,
                         const int8_t *pot_levels = nullptr,
                         int pot_levels_size = 15,
                         const int8_t *pot_opt_levels = nullptr,
                         int pot_opt_levels_size = 15) {
  // taking layer wgt dimension
  int *wgtDims = wgt->dims->data;
  int wgtWidth = wgtDims[0];
  int wgtDepth = wgtDims[1] * wgtDims[2] * wgtDims[3];
  int8_t *wgtData = wgt->data.int8;
  auto *affine_quantization =
      reinterpret_cast<TfLiteAffineQuantization *>(wgt->quantization.params);
  float *wgt_scales = affine_quantization->scale->data;

  // loop through each of the filter of the wgts
  for (int fil_no = 0; fil_no < wgtWidth; ++fil_no) {
    // check all the weight in a filter is within the pot_levels
    int filDims[4] = {1, wgtDims[1], wgtDims[2], wgtDims[3]};
    bool is_this_potFilter = check_pot_weights(
        wgtData + fil_no * wgtDepth, filDims, pot_levels, pot_levels_size);
    if (is_this_potFilter) {
      // for testing: method 2: with int8-> pot_int8 -> pot_opt_int
      // requantize the weights
      wgt_scales[fil_no] = static_cast<float>(requantize_potInt(
          wgtData + fil_no * wgtDepth, static_cast<double>(wgt_scales[fil_no]),
          wgt_pot + fil_no * wgtDepth, filDims, -pot_levels[0],
          -pot_opt_levels[0], pot_levels, pot_levels_size, pot_opt_levels,
          pot_opt_levels_size));
      // check if the requantized weights are within the pot_opt_levels
      bool is_this_pot_optFilter =
          check_pot_weights(wgt_pot + fil_no * wgtDepth, filDims,
                            pot_opt_levels, pot_opt_levels_size);
      if (!is_this_pot_optFilter) {
        cout << "Error1: prepare_pot_weights(): Filter[" << fil_no
             << "] is this_pot_optFilter: " << is_this_pot_optFilter << endl;
        exit(1);
      }
    } else {
      // find the pot_levels for which int8_t values in the filter are within
      // the pot_levels
      // find the index pot_level
      int pot_level_index = find_pot_level_index(
          wgtData + fil_no * wgtDepth, filDims,
          static_cast<double>(wgt_scales[fil_no]), pot_levels, pot_levels_size);

      // for testing: method 2: with int8-> pot_int8
      wgt_scales[fil_no] = static_cast<float>(requantize_int8(
          wgtData + fil_no * wgtDepth, static_cast<double>(wgt_scales[fil_no]),
          wgt_pot + fil_no * wgtDepth, filDims, -pot_levels[0],
          -pot_levels[pot_level_index], pot_levels, pot_levels_size));
      is_this_potFilter = check_pot_weights(
          wgt_pot + fil_no * wgtDepth, filDims, pot_levels, pot_levels_size);
      if (!is_this_potFilter) {
        cout << "Error 2: prepare_pot_weights(): Filter[" << fil_no
             << "] is_this_potFilter: " << is_this_potFilter << endl;
        // find the unique values in the filter
        std::set<int8_t> unique_values(wgt_pot + fil_no * wgtDepth,
                                       wgt_pot + fil_no * wgtDepth + wgtDepth);
        // print unique values
        cout << "Unique values in Filter[" << fil_no << "]: ";
      ...
      // pot_int8 -> pot_opt_int
      // requantize the weights
      wgt_scales[fil_no] = static_cast<float>(requantize_potInt(
          wgt_pot + fil_no * wgtDepth, static_cast<double>(wgt_scales[fil_no]),
          wgt_pot + fil_no * wgtDepth, filDims, -pot_levels[pot_level_index],
          -pot_opt_levels[pot_level_index], pot_levels, pot_levels_size,
          pot_opt_levels, pot_opt_levels_size));
      // check if the requantized weights are within the pot_opt_levels
      bool is_this_pot_optFilter =
          check_pot_weights(wgt_pot + fil_no * wgtDepth, filDims,
                            pot_opt_levels, pot_opt_levels_size);
      if (!is_this_pot_optFilter) {
        cout << "Error 3: prepare_pot_weights(): Filter[" << fil_no
             << "] is this_pot_optFilter: " << is_this_pot_optFilter << endl;
        exit(1);
      }
    }
...
#elif defined(APOT)
        w0 = apot_weight_encoding(w0);
        w1 = apot_weight_encoding(w1);
        w2 = apot_weight_encoding(w2);
        w3 = apot_weight_encoding(w3);
#else // MSQ
// TODO: msq_weight_encoding
        w0 = msq_weight_encoding(w0);
        w1 = msq_weight_encoding(w1);
        w2 = msq_weight_encoding(w2);
        w3 = msq_weight_encoding(w3);
#endif
        wb0.push_back(w0);
        wb1.push_back(w1);
        wb2.push_back(w2);
...
// =======================================================
// Weight PreProcessing for Power of Two Quantization 
// For QKeras, Step 3 is only required
// =======================================================
// Step1 : Converting weights from int8 --> pot_int
// Step2 : requantize the Bias
// Step3 : Encode (pot_int^(e)) and compress. see prepare_weights() in util_prep.h
// =======================================================

// Step1 : Converting weights from int8 --> pot_int
#ifdef APOT
  // int8 levels
  int8_t apot_levels[15] = {-127, -102, -76, -51, -38, -25, -13, 0,
                            13,   25,   38,  51,  76,  102, 127};
  // pot_int levels
  int8_t apot_levels_opt[15] = {-10, -8, -6, -4, -3, -2, -1, 0,
                                1,   2,  3,  4,  6,  8,  10};

  TfLiteTensor *filterPot;
  std::vector<int8_t> wgt_pot;
  std::vector<float> wgt_scales_orig;
  int weight_tensor_index = inputs_[i][1];
  filterPot = &context->tensors[weight_tensor_index];
  int ww = filterPot->dims->data[0];
  int wd = filterPot->dims->data[1] * filterPot->dims->data[2] *
           filterPot->dims->data[3];
  wgt_pot.resize(ww * wd);

  // declare a vector to copy the original weight scales
  // these scales will be used to requantize the bias
  wgt_scales_orig.resize(filterPot->dims->data[0]);
  auto *affine_quantization_test = reinterpret_cast<TfLiteAffineQuantization *>(
      filterPot->quantization.params);
  float *wgt_scales = affine_quantization_test->scale->data;
  for (int f = 0; f < filterPot->dims->data[0]; f++) {
    wgt_scales_orig[f] = wgt_scales[f];
  }

  // prepare the weights with the apot_levels_opt
  prepare_pot_weights(filterPot, &wgt_pot[0], apot_levels, 15, apot_levels_opt,
                      15);
#elif defined(MSQ)
  // TODO: update the msq_levels and msq_levels_opt
  // int8 levels
  int8_t msq_levels[15] = {-127, -102, -76, -51, -38, -25, -13, 0,
                            13,   25,   38,  51,  76,  102, 127};
  // pot_int levels
  int8_t msq_levels_opt[15] = {-10, -8, -6, -4, -3, -2, -1, 0,
                                1,   2,  3,  4,  6,  8,  10};
  ...
  // prepare the weights with the msq_levels_opt
  prepare_pot_weights(filterPot, &wgt_pot[0], msq_levels, 15, msq_levels_opt,
                      15);
#endif
  ...
  // // check bias is nullpointer or not
  // if (bias == nullptr) {
  //   assert("Error: Prepare_CONV2D_INT8(): Bias is nullptr, Not Supported");
  // }
  // copy bias to biases vector
  biases.resize(bias->dims->data[0]);
  biases.assign(bias->data.i32, bias->data.i32 + bias->dims->data[0]);
  int *dims = filter->dims->data;
  int inpZeroPoint = -input->params.zero_point;
#if defined (APOT) || defined (MSQ) 
  // Step2 : requantize the bias with the new weight scales
  for (int f = 0; f < bias->dims->data[0]; f++) {
    // dequantize
    double dbias =
        static_cast<double>(bias->data.i32[f]) *
        static_cast<double>(wgt_scales_orig[f] * input->params.scale);
    // requantize
    double rbias =
        dbias / static_cast<double>(wgt_scales[f] * input->params.scale);
    biases[f] = (int)round(rbias);
  }
  // Step3 : Encode (pot_int^(e)) and compress. see prepare_weights() in util_prep.h
  prepare_weights(&wgt_pot[0], filter->dims->data, wb0, wb1, wb2, wb3, wt_sum1,
                  wt_sum2, wt_sum3, wt_sum4, -input->params.zero_point,
                  &biases);
#else // QKeras
  // Step3 : Encode (pot_int^(e)) and compress. see prepare_weights() in util_prep.h
