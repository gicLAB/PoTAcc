...
void Load_Weight_Data(acc_container &drv, int free_buf, int8_t *results,
  ...
  if (tileOpt) {
    ...
    int w_dex = (drv.w_c / 4);
    int data_length = rcols_step * rdepth_step; // total no. of weights
    int rcolnoDiv4 = rcols_step / 4;
    int rdepthDiv8 = rdepth_step / 8; // 4-bit PoT
    int minWgtBlockNo =
    ...
    int offset =
        drv.t->layer_wgt_offsets[drv.t->layer] + drv.t->wgt_tile_offset;

    drv.mdma->multi_dma_change_start_4((DMA_SCRATCH_SIZE_4 + offset) * 4);
    drv.mdma->dmas[0].dma_start_send(data_length / 32); // 4-bit weight
    drv.mdma->dmas[1].dma_start_send(data_length / 32); // 4-bit weight
    drv.mdma->dmas[2].dma_start_send(data_length / 32); // 4-bit weight
    drv.mdma->dmas[3].dma_start_send(data_length / 32); // 4-bit weight
    drv.mdma->multi_dma_wait_send();
    drv.t->wgt_tile_offset += data_length / 32; // 4-bit weight
...
void TileGEMM(acc_container &drv, int output_stride, int depth, int rdepth,
  ...
  int acc_individual_weight_buffer_size = WGT_BUF_LEN * 16; // 16*1*WGT_BUF_LEN
  if ((maxWgtBlocks * 4 * rdepth) >
      acc_individual_weight_buffer_size) { // 16*1*WGT_BUF_LEN
  ...
  for (int r = 0; r < rrows; r += row_inc) {
    ...
    for (int c = 0; c < rcols; c += col_inc) {
      ...
      // check for critical rcols_step and rdepth : need to review the code
#if 1
