...
// preload the weight in the DMA region before the inference starts.
...
// Number of GEMM units in the accelerator
// by default simulation will run for PYNQ-Z2
// to enable the simulatin of KRIA
// add --cxxopt='-DKRIA' in the task command ./tensorflow/.vscode/tasks.json

#ifdef KRIA
#define VMM_COUNT 8
#define ENABLE_URAM
#define WGT_BUF_LEN (8192 * 3)
#define INP_BUF_LEN 4096
#define GINP_BUF_LEN ((8192 * 2))
#else // PYNQ-Z2
#define VMM_COUNT 4
#define WGT_BUF_LEN 4096
...
#define BRAM_DATAWIDTH 16
#define URAM_DATAWIDTH 64
#define ACC_OUT_SIZE 16 // output size of accelerator

// select PoT quantization and traget shift-PE design
// define with the flag during binary build [-DQKERAS, -DAPOT, -DMSQ]

//ipw: intermediate product width
#define PROD_DATA_WIDTH_QKERAS 15
#define PROD_DATA_WIDTH_8x4 12
#define PROD_DATA_WIDTH_MSQ 11
#define PROD_DATA_WIDTH_APOT 12
...
typedef struct byteToUF {
  ...
  void unpack(acc_dt a1[], acc_dt a2[], acc_dt a3[], acc_dt a4[], int idx) {
    // Unpack 4x32 bit data into 4x32 bit arrays
  ...
  void unpack16(sc_int<16> a1[], sc_int<16> a2[], sc_int<16> a3[],
                sc_int<16> a4[], int idx) {
    // Unpack 4x32 bit data into 4x16 bit arrays
    a1[idx] = data.range(15, 0);
    a1[idx + 1] = data.range(31, 16);
    a2[idx] = data.range(47, 32);
    a2[idx + 1] = data.range(63, 48);
    a3[idx] = data.range(79, 64);
    a3[idx + 1] = data.range(95, 80);
    a4[idx] = data.range(111, 96);
    a4[idx + 1] = data.range(127, 112);
  }
} bUF;
